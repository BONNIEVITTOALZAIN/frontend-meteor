...
const Ruangan = () => {
  ...
  const fetchRuangan = async () => {
    try {
      setLoading(true);
      const res = await api.get("/admin/ruangan");
      setRuangan(res.data);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nama.trim()) {
      Swal.fire("Gagal", "Nama ruangan wajib diisi!", "error");
      return;
    }

    try {
      if (editMode) {
        await api.put(`/admin/ruangan/${currentRuangan.id}`, formData);
        Swal.fire("Berhasil!", "Data ruangan telah diperbarui.", "success");
      } else {
        await api.post("/admin/ruangan", formData);
        Swal.fire("Berhasil!", "Data ruangan telah ditambahkan.", "success");
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      Swal.fire("Berhasil!", "Data ruangan telah dihapus.", "success");
      fetchRuangan();
    } catch (err) {
  ...
  const currentItems = filteredRuangan.slice(indexOfFirst, indexOfLast);
